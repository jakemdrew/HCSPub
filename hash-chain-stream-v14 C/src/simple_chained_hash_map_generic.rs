    ...
    V: Clone,
{
                ...
                ($i:expr) => {{
                    ...
                    match &mut shard.slots[idx] {
                        None => {
                            ...
                            shard.len += 1;
                            if shard.load_factor_exceeded() { self.resize_shard(sidx); }
                            return None;
                        }
                        Some(slot) => {
    ...
    fn resize_shard(&mut self, sidx: usize) {
        ...
        shard.slots = std::iter::repeat_with(|| None).take(new_size).collect::<Vec<_>>();
        ...
        for slot_opt in old_slots.into_iter() {
            if let Some(slot) = slot_opt {
                ...
                if (slot.full_hash & DELETED_HASH) != 0 { continue; }
