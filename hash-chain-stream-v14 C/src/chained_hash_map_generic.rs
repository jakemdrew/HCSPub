...
// CHANGED: use top-bit as tombstone flag (mask)
    ...
    V: Send + Sync + Clone + 'static,
{
    ...
    // CHANGED: branchless sanitize; clear tombstone flag on live hashes
    #[inline]
    ...
    fn shard_index(&self, key: &K) -> (u32, usize) {
        ...
        let hash_trunc = Self::sanitize_hash(raw); // keep flow identical, just branchless
    ...
    fn insert_with_stripe_lock(&self, full_hash: u32, key: K, value: V, shard: &Shard<K, V>) -> Option<V> {
        let full_hash = Self::sanitize_hash(full_hash); // ensure sanitized if caller passed raw
        loop {
                ...
                macro_rules! check_slot {
                    ($i:expr) => {{
                        ...
                        match &mut data.slots[idx] {
                            ...
                            Some(slot) => {
                                // CHANGED: tombstone test via bit check
    ...
    fn resize_shard(&self, shard: &Shard<K, V>) {
        ...
        for slot_opt in old_slots.into_iter().flatten() {
            // CHANGED: skip tombstones via bit test
            if (slot_opt.full_hash & DELETED_HASH) != 0 { continue; }
            ...
            macro_rules! check_slot {
                ($i:expr) => {{
                    ...
                    match &data.slots[idx] {
                        ...
                        Some(slot) => {
                            // CHANGED: ignore tombstones via bit test
                            if (slot.full_hash & DELETED_HASH) == 0 && slot.full_hash == hash_trunc && slot.key == *key {
            ...
            macro_rules! check_slot {
                ($i:expr) => {{
                    ...
                    match &mut data.slots[idx] {
                        None => return None, // true empty terminates
                        Some(slot) => {
                            // CHANGED: require live (no flag) before match
                            if (slot.full_hash & DELETED_HASH) == 0 && slot.full_hash == hash_trunc && slot.key == *key {
                                let old_value = slot.value.clone();
                                // mark tombstone by setting the flag (value bits ignored)
